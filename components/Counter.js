...
class Counter extends Component {
  render() {
    const { increment1,increment2,increment3, counter,src,title,time,content } = this.props
      ...
      <section refs="">
          ...
          <div className="section-text  ">
            ...
            <div className="clearfix ">
              ...
              <span className="pull-right"  style={{color:'red'}} onClick={this.props.ince}>
                <i className="glyphicon glyphicon-heart"></i>
                 {this.props.counter}
              </span>
              <a className="pull-right" onClick={this.displayContent}  style={{marginRight:'20px'}} >评论</a>
            </div>
          </div> 
          {
      ...
      <div className="pinglun form-horizontal">
        <div className="form-group add_pl" >
          <div className="pl-wrap pull-left"  >
            <input className="form-control" ref="add" name="pinglun" type="text" placeholder="这里可以添加评论的哦~" />
          </div>
          <input className="btn btn-primary"  type="submit" onClick={this.AddContent.bind(this)} value="提交" />
        </div>
          ...
          data[newId].map(function (items,keys) {
        ...
        </div>
            {
              that.state.display ?<div className="form-group add_pl" >
                                <div className="pl-wrap pull-left"  >
                                  <input className="form-control" ref="addHf" name="pinglun" type="text" placeholder="这里可以添加评论的哦~" />
                                </div>
                                <input className="btn btn-warning"  type="submit" onClick={that.handleAddHf.bind(that)} value="提交" />
                              </div>:null
                   }      
    </div>
    )
